...
**Scope decision:** wire into `solveTISE` itself (shared by `main.cpp`/`H-BoundStates`), not only the separate config-driven `tise_solver_main.cpp` CLI. This was an explicit choice — `tise_solver_main.cpp` is architecturally capable of doing this work in isolation too (leaving `H-BoundStates`'s exact numeric baseline untouched), but the shared-library option was chosen deliberately, accepting that `H-BoundStates`'s printed hydrogen eigenvalues will change as a result (see "Baseline impact" below) — a deliberate physics improvement, not a regression.
...
- `bSplinesTouchingX` at a domain-**boundary** singularity returns an entire cluster of `order` indices, not 1 — hand-verified for `order=4`/`order=8` in `engineer-a-plan-A4b.md:252-257`, and this doc's own worked hydrogen example below (§"Baseline impact") extends the same derivation to `order=12`.
...
For every `Singular` join (domain-edge or interior), `bSplinesTouchingX(nNodesActual, order, grid, join.x)` contributes its returned indices to a fill-time drop-set, unioned with the classic `{1}`. `nBSplines` itself is always excluded from this fill-time set (even if a right-edge singular cluster would otherwise include it) — continuum construction needs `B_N`'s raw column filled, not dropped; it's excluded from the diagonalization by truncation instead, generalizing the existing `{1}`/`nEn+1`/truncate pattern to an arbitrary-size cluster. A right-edge singularity (where `matchAsymptotic`'s flat-asymptote assumption at `R` would no longer hold) emits a non-fatal `stderr` warning rather than failing the solve.
...
- `strategicKnotsFromJoins`: `Singular → extra=0` → no knot. **Gap 1 is a byte-identical no-op for this potential** — grid stays the plain 51-point uniform grid, spacing 2.0.
- `nBSplines = 51+12-2 = 61` (unchanged).
- `bSplinesTouchingX(51, 12, grid, 0)`: for `bs=1..12`, `lo=knotAt(bs-12)` clamps to `grid.front()=0` in every case, `hi=knotAt(bs)≥0` → all satisfy `lo≤0≤hi`. For `bs=13`: `lo=knotAt(1)=grid[1]=2>0` → fails. **Cluster = `{1,...,12}`, exactly `order=12` indices.**
- Combined drop-set: `{1,...,12,61}` (13 total) vs. today's `{1,61}` (2 total). **`nEn`: 59 → 48.**
- `main.cpp`'s printed "accurate eigenvalues" count (breaks at the first exceeding `1e-10` error vs. analytic hydrogen) is expected to stay **4** — removing far-from-origin basis freedom doesn't touch the first few bound states — but the literal 16-digit values will change (very likely *more* accurate, since the removed B-splines sampled closest to the `-1/x` divergence). Re-verified against a real run in Task 6 of the implementation plan this doc accompanies; `docs/planning/engineer-a-plan-misc.md:191,193` holds the old values pending that update.
