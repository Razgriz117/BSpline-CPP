# Test report: `tests/harmonic_oscillator.yaml` — iteration 3

**Verified against:** the quantum harmonic oscillator, $E_n = n + \tfrac12$, Hermite-function eigenstates (exact closed form).
**Branch / commit:** `TISE-Generalization` @ `f4e8359`. **Previous:** `8236239`, `9d0f04b`. Solver output for this YAML is byte-identical across all three commits; only the plots changed (see §7).
**Date:** 2026-09-11.
**Verdict:** **PASS (unchanged).** Energies and eigenfunctions match the exact solution to better than $10^{-6}$ for the first eight states ($n = 0$–7) and to $10^{-4}$ through $n = 10$. The degradation above that is the 0.5-bohr node spacing, not a solver defect: rerunning with 161 nodes moves the $10^{-6}$ frontier from $n = 7$ to $n = 35$.
...
## 7. What changed at f4e8359 for this test

* **Output data:** byte-identical to both previous commits (no interior joins, no asymptote classification, no continuum).
* **YAML:** `visualization.bound_states_squared: true` added (commit `5cd28e4`). `analysis.py` now plots raw $\phi_n(x)$ by default and this flag opts the oscillator back into $|\phi_n|^2$ for "bound" states. With the continuum disabled every computed state counts as bound, so all 87 PNGs are still densities — the commit message records that a naive $E<0$ criterion would have produced no squared plots at all for this test, since every oscillator level is positive. `figures/output_plots_f4e8359.png` (panels 4–5) shows the $n=0$ and $n=2$ densities with the expected peak heights $1/\sqrt\pi = 0.564$ and $0.37$.
* **Round-1/2 recommendations status:** unchanged — grid refinement, plot gating, `computeEAcc`, non-uniform grid exposure and an $n+\tfrac12$ assertion are all still open. This test remains the cleanest place to add the last of these: `abs(E_n − (n+½)) < 1e-6` for $n \le 7$ at 81 nodes.
