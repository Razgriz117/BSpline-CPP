...
x,y=state(t,1); ax[0].plot(x,np.sqrt(2/20)*np.sin(np.pi*x/20)*(x<20),'k-',lw=2.5,alpha=.35,label='exact  n=1 box state'); ax[0].plot(x,np.sign(y[50])*y,'C0-',lw=1,label='solver state 1'); ax[0].set_title(f'state 1: E={E[0]:.5f}  (exact {box[0]:.5f})',fontsize=9); ax[0].legend(fontsize=7)
...
ax[1].plot(x,fe,'k-',lw=2.5,alpha=.35,label='exact  Coulomb-side state 1'); ax[1].plot(x,np.sign(y[np.argmax(np.abs(y))])*np.sign(fe[np.argmax(np.abs(y))])*y,'C0-',lw=1,label='solver state 3'); ax[1].set_title(f'state 3: E={E[2]:.5f}  (exact {coul[0]:.5f})',fontsize=9); ax[1].legend(fontsize=7)
ax[2].plot(range(1,13),E[:12],'o',label='solver f4e8359'); ax[2].plot(range(1,13),exact[:12],'k_',ms=14,label='exact'); ax[2].plot(range(1,13),proper[:12],'x',label='B-splines, mult-7 knot at 20 + drop 1'); ax[2].set_xlabel('state'); ax[2].set_ylabel('E'); ax[2].legend(fontsize=7); ax[2].set_title('spectrum',fontsize=9)
...
cinfo="continuum construction skipped by solver (rightEdgeSingular)"; import os; cinfo_files=[f for f in os.listdir(D/t/"tise") if f.startswith("continuum") or f.startswith("phase")]
rep[t]=dict(rows=rows,wf=wf,continuum=cinfo, continuum_files=cinfo_files, n_exact_found=len(exR))
...
x,y=state(t,5); k=np.sqrt(2*exR[4]); fe=np.array([float(mp.coulombf(0,1/k,k*(100-xx))) for xx in x]); fe/=np.sqrt(np.trapezoid(fe**2,x)); s5=np.sign(np.dot(y,fe))
ax[1].plot(x,fe,'k-',lw=2.5,alpha=.35,label='exact'); ax[1].plot(x,s5*y,'C0-',lw=1,label='solver state 5'); ax[1].legend(fontsize=7); ax[1].set_title(f'state 5: E={E[4]:.6f} (exact {exR[4]:.6f})',fontsize=9)
...
ax[1].plot(range(1,13),E[:12]-e_tap[:12],'o-',label='solver − FD(tapered V)  [8236239 behaviour]'); ax[1].plot(range(1,13),E[:12]-e_raw[:12],'s-',label='solver − FD(raw V)'); ax[1].axhline(0,color='k',lw=.5); ax[1].set_yscale('symlog',linthresh=1e-7); ax[1].legend(fontsize=7); ax[1].set_xlabel('state'); ax[1].set_title('energy differences',fontsize=9)
