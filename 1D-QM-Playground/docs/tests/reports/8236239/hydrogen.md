# Test report: `tests/hydrogen.yaml` — iteration 2
...
**Date:** 2026-09-04.
**Verdict:** **PASS (unchanged).** 2p–5p energies and radial functions are reproduced to $10^{-6}$ or better (2p, 3p to $10^{-13}$); higher states show textbook confinement by the $r = 100$ wall. Continuum wavefunctions are the exact Coulomb waves to $< 1$ %. The reported phase shifts are self-consistent to $10^{-7}$ but are "flat-asymptote" values at $R = 100$, not Coulomb phase shifts — a documented limitation, quantified here at 0.02–0.07 rad. The `dδ/dE` column is unusable.
...
* **Output data:** byte-identical to 9d0f04b. The singular point $r=0$ is a domain edge, which the old and new grid builders treat identically (full knot multiplicity there; drop $B_1$).
* **New diagnostics — and they are right.** `warnings.json` now says "7 of 59 computed states are below E=0.0", and `checkWellContainment` flags exactly bound states 5, 6, 7 (6p, 7p, 8p; $\psi'(100) = 0.0025, 0.0104, 0.0188$) as "colliding with the outer wall". That is precisely the box-confinement boundary identified in round 1 (§4.1: 5p accurate to $10^{-6}$, 6p to $10^{-3}$, 7p/8p strongly shifted). The diagnostic reproduces the analysis without knowing the exact answer, which is what a diagnostic is for. State 4 (5p, error $10^{-6}$) is correctly *not* flagged with the default tolerance $10^{-3}$.
* **Round-1 recommendations status:** (1) name the test `hydrogen_l1` / add an ℓ = 0 companion — not done; (2) Coulomb-function matching for Coulomb tails — not done (still explicitly deferred in the release plan); (3) larger box / non-uniform grid — not done; (4) fix $d\delta/dE$ — not done (the +211 at $E=0.1$ is still there); (5) plot only meaningful states — not done; (6) regression against $-1/2n^2$ — not done (the e2e test checks `warnings.json` content only).
