# Test report: `tests/harmonic_oscillator.yaml` — iteration 2
...
## 7. What changed at 8236239 for this test

* **Output data:** byte-identical to 9d0f04b (no interior joins; the potential is capped at the box edge so no asymptote classification runs).
* **New diagnostics:** `warnings.json`: "0 of 89 computed states are below E=0.0" — correct, since $V \ge 0$ and the threshold is 0. Note that for a confining potential like this one "below E=0" is not the same as "bound" (every oscillator state is bound); the informational message is accurate but the label could mislead.
* **Round-1 recommendations status:** (1) raise `n_nodes` — not done (test still certifies $n \le 7$); (2) plot only trustworthy states — not done (87 PNGs); (3) fix `computeEAcc` — not done; (4) non-uniform grid — the machinery now reaches `tise_solver`, but nothing in the YAML schema lets a user request it for a smooth potential; (5) automated regression against $n+\tfrac12$ — not done (the e2e test loads the YAML but asserts only that the run succeeds).
