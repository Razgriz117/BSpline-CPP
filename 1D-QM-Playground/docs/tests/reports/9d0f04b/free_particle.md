# Test report: `tests/free_particle.yaml`
...
**Date:** 2026-09-04.
**Verdict:** **PASS.** Bound energies, eigenfunctions, normalisation, continuum states, and phase shifts agree with the exact solution to machine precision for every state the basis can resolve.

Companion files in this directory: `known_solutions_report.html` (all four tests in one document, with figures embedded) and `verify_known_solutions.py` (the script that produced every reference number and figure cited below). Figures referenced here live in `figures/`.
